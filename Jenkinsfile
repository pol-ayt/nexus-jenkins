pipeline {
  ...
  stages {
    stage('Prep Java & Maven') {
      steps {
        sh 'export PATH="$JDK_HOME/bin:$MAVEN_HOME/bin:$PATH"; java -version; mvn -v'
      }
    }
    ...
    stage('Write Maven settings.xml (safe)') {
      steps {
        withCredentials([usernamePassword(credentialsId: 'nexus-creds', usernameVariable: 'NUSER', passwordVariable: 'NPASS')]) {
          // Create settings.xml via shell so $NUSER/$NPASS expand in Bash, not in Groovy
          sh '''
...
XML
            # Optional: show sanitized view (mask password)
            sed -e "s#<password>.*</password>#<password>***</password>#g" settings.xml | head -n 50
          '''
        }
      }
    }
    ...
    stage('Build & Publish (deploy)') {
      steps {
        sh 'export PATH="$JDK_HOME/bin:$MAVEN_HOME/bin:$PATH"; mvn -s settings.xml -DskipTests clean deploy'
      }
    }

    stage('Fetch from Nexus (safe curl)') {
      steps {
        script {
          ...
          echo "Downloading artifact from: ${downloadUrl}"
        }
        withCredentials([usernamePassword(credentialsId: 'nexus-creds', usernameVariable: 'NUSER', passwordVariable: 'NPASS')]) {
          // IMPORTANT: single-quoted Groovy string; use $NUSER/$NPASS in the shell
          sh 'curl -fSL -u "$NUSER:$NPASS" -o app.jar "$downloadUrl"'
          sh 'ls -lh app.jar'
        }
      }
    }
